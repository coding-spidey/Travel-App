...
app.listen(8081, function () {
 ...
 async function extractCoordinate(destination, travelDate, endDate) {
     ...
     await getWeather(lat, lon, travelDate, endDate)
 }
 async function getWeather(lat, lon, travelDate, endDate) {
     const weatherKey = process.env.WEATHER_KEY
     const resp = await fetch(`https://api.weatherbit.io/v2.0/current?&lat=${lat}&lon=${lon}&key=${weatherKey}&start_date=${travelDate}&end_date=${endDate}`)
 ...
 app.post('/data',async (req,res)=>{
     ...
     const endDate = req.body.returningDate
     await extractCoordinate(destination, travelDate, endDate)
